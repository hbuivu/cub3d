...
void	calc_step(t_main *main)
{
	if (float_is(main->calc->angle, main->n_angle) || float_is(main->calc->angle, main->s_angle))
		...
		main->calc->stepx = -1;
	if (float_is(main->calc->angle, 0) || float_is(main->calc->angle, M_PI))
		main->calc->stepy = 0;
...
void	raycast(t_main *main)
{
	...
	while (x < main->calc->pln_width)
	{
		...
		if (float_is(c->angle, 0) || float_is(c->angle, M_PI))
		{
			c->deltay = 0;
			c->deltax = 64;
			if (c->stepx == 1)
			...
			(main->map[(int)(c->col_inty / c->upg)][(int)(c->col_int / c->upg)] != '1'))
		{
			...
			if (x == 117)
			{
				...
				printf("c->col_inty: %lf\n", c->col_inty);
				printf("c->col_int: %lf\n", c->col_int);
				printf("418: check row: %i check column: %i\n", (int)(c->col_inty / c->upg), (int)(c->col_int / c->upg));
				printf("mw: %i mh: %i\n", main->map_width, main->map_height);
			}
		}
		...
		while ((int)(c->row_int / c->upg) < main->map_height &&
			...
			main->map[(int)(c->row_int / c->upg)][(int)(c->row_int / c->upg)] != '1')
		{
		...
		if (c->dist_col <= c->dist_row)
			c->cor_dist = c->dist_col * cos(c->fov / 2);
		else
			c->cor_dist = c->dist_row * cos(c->fov / 2);
